...
class RPN(nn.Module):
	...
	@property
	def loss(self):
		return self.cross_entropy + self.loss_box * 100


	def forward(self, pyramid_network, image_height, image_width, is_training=True, gt_boxes=None, gt_masks=None):
		...
		# proposal layer
		rois = self.proposal_layer(rpn_cls_prob, rpn_bbox_pred, image_height, image_width, stride, anchor_scales, is_train=self.training)
...
class ObjectDetectionNetwork(nn.Module):
	...
	def forward(self, features, rois, roi_data=None, is_training=True):
		self.training = is_training
...
class Mask(nn.Module):
	def __init__(self):
		...
		self.mask_conv_7 = nn.Conv2d(256, 28, 1)

		#self.softmax2d = nn.Softmax2d()

		# loss
		self.mask_loss = Variable(torch.zeros(1))

		self.training = True



	def forward(self, cls_prob_pos, rois_pos, aligned_features_pos, index_keep, image_height, image_width, gt_boxes=None, gt_masks=None, roi_data=None, is_training=True):
		self.training = is_training
		...
		keypoints = self.mask_conv_7(features)
		#keypoints = self.softmax2d(keypoints)
	...
	def build_loss(self, keypoints, rois_pos, index_keep, gt_boxes, gt_masks, image_height, image_width, roi_data):
		...
		# keypoints = keypoints.permute(0,2,3,1).contiguous().view(-1,29)
		# keypoints = F.softmax(keypoints)
		keypoints = keypoints.view(-1,2*448*448)
		#print(keypoints)
		keypoints = F.softmax(keypoints)

		#mask_label_onehot = torch.zeros(mask_label.size()[0],29).scatter_(1,mask_label.data.cpu(),1)
		#print(torch.sum(mask_label_onehot, 0))
		mask_label = mask_label.view(-1,2*448*448)

		#mask_weight = Variable(torch.cat((torch.ones(mask_label.size()[0],1)*0.001, torch.ones(mask_label.size()[0],28)*5.0),1)).cuda()

		mask_loss = -torch.sum(torch.log(keypoints) * mask_label)/ mask_label.size()[0]
		#mask_loss = -torch.sum(torch.log(keypoints) * mask_label) / float(max(torch.sum(torch.gt(mask_label,0.0)).data[0], 1.0))
		# print(torch.sum(mask_label))
		# print(torch.sum(torch.log(keypoints) * mask_label))
		# print(torch.sum(torch.gt(mask_label, 0)))
		# print(torch.sum(torch.gt(mask_label, 1)))
		#mask_loss = F.cross_entropy(keypoints, mask_label)
...
class Network(nn.Module):
	...
	def forward(self, input, image_height, image_width, is_training=True, gt_boxes=None, gt_masks=None):
		self.training = is_training
		pyramid = self.pyramid_network(input)
		rois = self.RPN(pyramid, image_height, image_width,  self.training, gt_boxes, gt_masks)
		if self.training:
		...
		cls_prob, bbox_pred, aligned_features = self.odn(pyramid, rois, roi_data, self.training)
		...
		cls_prob_pos, rois_pos, aligned_features_pos, index_keep, is_empty = self.pos_filter_layer(cls_prob, rois,
																						 aligned_features)
		if not is_empty:
			keypoints = self.mask(cls_prob_pos, rois_pos, aligned_features_pos, index_keep, image_height, image_width, gt_boxes, gt_masks, roi_data, self.training)
		...
		if not is_empty:
			return rois_pos.data.cpu().numpy(), cls_prob_pos.data.cpu().numpy(), keypoints.data.cpu().numpy()
		else:
			return rois_pos, cls_prob_pos, keypoints.data.cpu().numpy()
