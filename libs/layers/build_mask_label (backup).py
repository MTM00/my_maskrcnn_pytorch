...
def build_mask_label(rois_pos, gt_boxes, gt_masks, index_keep, gt_assignment, image_height, image_width):
	...
	for j in range(coordinate.shape[0]):
		...
		for n in range(coor.shape[0]):
			mask_label_key_n = np.zeros((rois_height, rois_width, 2))
			if coor[n,2]-1 >= 0:
				mask_label_key_n[coor[n,1], coor[n,0], coor[n,2]-1] = 1
			mask_label = cv2.resize(mask_label_key_n,(448, 448), interpolation=cv2.INTER_AREA)
			mask_labels.append(mask_label)


	mask_labels = np.array(mask_labels).reshape((-1,14,448,448,2)).transpose((1,0,4,2,3))
