...
from ..boxes.cython_bbox import bbox_overlaps, bbox_intersections
...
def proposal_target_layer(rpn_rois, gt_boxes, image_height, image_width, _num_classes):
    """
    ...
    """
    #jittered_gt_boxes = _jitter_gt_boxes(gt_boxes, image_height, image_width)
    ...
    #                       np.hstack((zeros, np.vstack((gt_boxes, jittered_gt_boxes))))))
    ...
    # targets
    labels, rois, bbox_targets, bbox_inside_weights, gt_assignment_keep = _sample_rois(
        all_rois, gt_boxes, fg_rois_per_image, rois_per_image, _num_classes)
...
def _sample_rois(all_rois, gt_boxes, fg_rois_per_image, rois_per_image, num_classes):
    ...
    # only one class
    labels = np.ones((gt_assignment.shape[0],))
    ...
    gt_assignment_keep = gt_assignment[keep_inds]
    ...
    return labels, rois, bbox_targets, bbox_inside_weights, gt_assignment_keep
...
def _jitter_gt_boxes(gt_boxes, image_height, image_width, jitter=0.05):
    ...
    jittered_boxes[np.where(jittered_boxes[:, 0] <= 0)[0][0], 0] = 0
    jittered_boxes[np.where(jittered_boxes[:, 1] <= 0)[0][0], 1] = 0
    jittered_boxes[np.where(jittered_boxes[:, 2] >= image_width)[0][0],2] = image_width - 1
    jittered_boxes[np.where(jittered_boxes[:, 3] >= image_height)[0][0],3] = image_height - 1
