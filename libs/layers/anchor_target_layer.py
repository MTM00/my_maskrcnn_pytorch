...
def anchor_target_layer(rpn_cls_score, gt_boxes, image_height, image_width, stride, anchor_scales):
	...
	if DEBUG:
		...
		print('')
		print(('im_size: ({}, {})'.format(image_height, image_width)))
	...
	total_anchors = int(K * A)
		...
		# 	(all_anchors[:, 2] < image_width + _allowed_border) &  # width
		# 	(all_anchors[:, 3] < image_height + _allowed_border)  # height
		# )[0]
		#
		# if DEBUG:
	...
	anchors = clip_boxes(all_anchors, [image_height, image_width])
	...
	max_overlaps = overlaps[np.arange(len(labels)), argmax_overlaps]
